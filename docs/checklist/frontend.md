...
- [~] **Module 9 — Documents · Files** — `/documents` Library wired to real `GET/POST/DELETE /documents/files`: `useDocumentFiles` hook, **multipart upload** (client allowlist + 15 MB gate per guide §5), real per-folder counts, empty state, per-row delete; **verified live end-to-end** (uploaded a .txt → server stored + row rendered → deleted → back to empty). **Follow-up sub-slices:** Documents Sharing · E-signature · Templates; file open/preview via `url`; mentor role-gating
- [ ] Cross-cutting: wire the hardcoded Health `72` pill + notification `5` bell badge across all route shells to real data
