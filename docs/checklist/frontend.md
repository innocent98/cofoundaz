...
- [~] **Module 2 — Dashboard** — plumbing done: `X-Workspace-Id` header (cross-cutting, in client), `summary`+`activity` live (200), real greeting/workspace, crashes fixed (briefing 404, risks/opps shape). **Follow-up:** widgets still mask real values with static sample data — the summary now returns real health/mission/kpis (test account onboarded+assessed), so finish widget-by-widget against the real nested shapes
