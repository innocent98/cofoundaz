...
- [~] **Module 11 — Documents · Sharing** — `/documents/shared` wired to real `GET /documents/shares` (workspace overview, title joined from `GET /documents`), active-only filter, per-row **revoke** (`DELETE …/{share_id}`), access levels fixed to view/comment; `useDocumentShares` also has `createShare`. **Verified live end-to-end** (created a doc → shared → row rendered with joined title → revoked → filtered out). **Follow-up:** per-document Share UI (show one-time `link`); rich-documents list/CRUD; public `/shared/{token}` open
