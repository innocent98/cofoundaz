...
export default function FinanceHubApp() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7f5] text-[#2c3531] flex font-sans relative">
      ...
      {showNotification && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 bg-[#0e271f] text-white px-5 py-3 rounded-xl shadow-xl transition-all duration-300 border border-gray-700">
        ...
        <header className="sticky top-0 z-40 bg-white border-b border-gray-200/85 shadow-xs w-full">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span>/</span>
              <span className="font-semibold text-gray-900">Finance Hub</span>
            </div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 flex-1 w-full space-y-6">
          ...
          {activeNavTab === "Cash flow" && (
            <div className="space-y-6 animate-fadeIn">
              <div>
                <h1 className="text-3xl font-serif font-semibold text-gray-900 mb-1">Cash Flow Analytics</h1>
                <p className="text-sm text-gray-600">Monitor cash inflows, outflows, and net liquidity positions.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-2xs">
                  <p className="text-xs text-gray-500 font-medium uppercase">Total Inflows (M1)</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">₦1,600K</p>
                  <span className="text-xs text-emerald-600 flex items-center gap-1 mt-2 font-medium"><ArrowUpRight className="w-3.5 h-3.5" /> +12% from last month</span>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-2xs">
                  <p className="text-xs text-gray-500 font-medium uppercase">Total Outflows (M1)</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">₦5,220K</p>
                  <span className="text-xs text-rose-600 flex items-center gap-1 mt-2 font-medium"><ArrowDownRight className="w-3.5 h-3.5" /> -3% optimized</span>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-2xs">
                  <p className="text-xs text-gray-500 font-medium uppercase">Closing Balance (M6)</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">₦21,042K</p>
                  <span className="text-xs text-gray-500 mt-2 block">Stable projected horizon</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeNavTab === "Runway" && (
            <div className="space-y-6 animate-fadeIn">
              <div>
                <h1 className="text-3xl font-serif font-semibold text-gray-900 mb-1">Runway & Burn Rate</h1>
                <p className="text-sm text-gray-600">Track your operational longevity based on current cash reserves.</p>
              </div>
              <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                  <span className="bg-emerald-50 text-emerald-700 text-xs font-bold px-3 py-1 rounded-full border border-emerald-200">Healthy Buffer</span>
                  <h2 className="text-4xl font-serif font-bold text-gray-900 mt-3">11.6 Months</h2>
                  <p className="text-sm text-gray-500 mt-1">Estimated runway remaining at current average net burn of ₦3,350K/mo.</p>
                </div>
                <div className="w-full md:w-auto flex gap-3">
                  <button className="px-4 py-2.5 bg-gray-900 text-white rounded-xl text-sm font-medium hover:bg-gray-800 transition">Simulate Hiring Impact</button>
                </div>
              </div>
            </div>
          )}
          ...
          {activeNavTab === "Budgets" && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex justify-between items-center">
                <div>
                  <h1 className="text-3xl font-serif font-semibold text-gray-900 mb-1">Department Budgets</h1>
                  <p className="text-sm text-gray-600">Allocate and monitor spending limits across teams.</p>
                </div>
                <button className="bg-[#b89d5f] text-white px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-1.5 hover:bg-[#a68c4f]">
                  <Plus className="w-4 h-4" /> New Budget
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {["Engineering", "Marketing", "Operations", "Sales", "General & Admin"].map((dept, i) => (
                  <div key={i} className="bg-white p-5 rounded-2xl border border-gray-200 space-y-3 shadow-2xs">
                    <div className="flex justify-between items-center">
                      <h3 className="font-bold text-gray-900">{dept}</h3>
                      <span className="text-xs text-gray-400 font-medium">Q1 Allocation</span>
                    </div>
          ...
          {activeNavTab === "Invoices" && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex justify-between items-center">
                <div>
                  <h1 className="text-3xl font-serif font-semibold text-gray-900 mb-1">Invoices</h1>
                  <p className="text-sm text-gray-600">Manage client billings, payments, and receivables.</p>
                </div>
                <button className="bg-[#b89d5f] text-white px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-1.5 hover:bg-[#a68c4f]">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-2xs">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50/50 border-b border-gray-100 text-xs text-gray-400 font-bold uppercase">
                  ...
                  <tbody className="divide-y divide-gray-100 text-sm">
                    {["INV-2026-001", "INV-2026-002", "INV-2026-003"].map((inv, idx) => (
                      <tr key={idx} className="hover:bg-gray-50/50">
                        <td className="py-4 px-6 font-semibold text-gray-900">{inv}</td>
                        <td className="py-4 px-4 text-gray-600">Enterprise Partner {idx + 1}</td>
                        <td className="py-4 px-4 text-gray-500">Mar {10 + idx}, 2026</td>
                        <td className="py-4 px-4 font-medium text-gray-900">₦{(idx + 1) * 600},000</td>
                        <td className="py-4 px-6">
                          <span className="bg-emerald-50 text-emerald-700 text-xs font-semibold px-2.5 py-1 rounded-full border border-emerald-200">Paid</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {activeNavTab === "Expenses" && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex justify-between items-center">
                <div>
                  <h1 className="text-3xl font-serif font-semibold text-gray-900 mb-1">Expenses</h1>
                  <p className="text-sm text-gray-600">Track corporate card transactions and operational spend.</p>
                </div>
                <button className="bg-[#b89d5f] text-white px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-1.5 hover:bg-[#a68c4f]">
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-100 text-sm">
                    {["AWS Cloud Services", "Google Workspace", "Office Lease", "Notion Team"].map((vendor, idx) => (
                      <tr key={idx} className="hover:bg-gray-50/50">
                        <td className="py-4 px-6 font-semibold text-gray-900">{vendor}</td>
                        <td className="py-4 px-4 text-gray-600">Infrastructure & SaaS</td>
                        <td className="py-4 px-4 text-gray-500">Feb {15 + idx}, 2026</td>
                        <td className="py-4 px-6 font-medium text-gray-900">₦{(idx + 1) * 120},500</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {activeNavTab === "Financial model" && (
            <div className="space-y-6 animate-fadeIn">
              <div>
                <h1 className="text-3xl font-serif font-semibold text-gray-900 mb-1">Financial model</h1>
                <p className="text-sm text-gray-600 flex items-center gap-1.5 font-medium">
                  <Sparkles className="w-4 h-4 text-[#b89d5f]" />
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-1.5 bg-gray-200/60 p-1 rounded-xl w-fit border border-gray-300/50">
                  <button
                    onClick={() => setActiveSubTab("pnl")}
                    className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all cursor-pointer ${
                      activeSubTab === "pnl" ? "bg-white text-gray-900 shadow-xs font-semibold" : "text-gray-600 hover:text-gray-900"
                    }`}
                  >
                    P&L
                  </button>
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all cursor-pointer ${
                      activeSubTab === "cashflow" ? "bg-white text-gray-900 shadow-xs font-semibold" : "text-gray-600 hover:text-gray-900"
                    }`}
                  >
                    Cash Flow
                  </button>
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all cursor-pointer ${
                      activeSubTab === "balancesheet" ? "bg-white text-gray-900 shadow-xs font-semibold" : "text-gray-600 hover:text-gray-900"
                    }`}
                  >
                    Balance Sheet
                  </button>
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 shadow-2xs transition-colors cursor-pointer active:scale-95 w-fit"
                >
                      ...
                      <tbody className="divide-y divide-gray-100 text-sm">
                        {pnlData.map((row, index) => (
                          <tr key={index} className="hover:bg-gray-50/60 transition-colors">
                            <td className="py-4 px-6 font-bold text-gray-900">{row.item}</td>
                            <td className="py-4 px-4 text-right text-gray-600">{row.m1}</td>
                            <td className="py-4 px-4 text-right text-gray-600">{row.m2}</td>
                            <td className="py-4 px-4 text-right text-gray-600">{row.m3}</td>
                            <td className="py-4 px-4 text-right text-gray-600">{row.m4}</td>
                            <td className="py-4 px-4 text-right text-gray-600">{row.m5}</td>
                            <td className="py-4 px-6 text-right text-gray-600">{row.m6}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          {activeNavTab === "Integrations" && (
            <div className="space-y-6 animate-fadeIn">
              <div>
                <h1 className="text-3xl font-serif font-semibold text-gray-900 mb-1">Integrations</h1>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {integrations.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="bg-white p-6 rounded-2xl border border-gray-200/80 shadow-xs flex flex-col justify-between space-y-6">
                      <div className="flex items-start gap-3.5">
                        <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center font-bold text-gray-800 text-sm border border-gray-200/60 flex-shrink-0">
                          {item.initials}
                        </div>
                        <div>
                      ...
                      <button
                        ...
                        className={`w-full py-2.5 px-4 rounded-xl text-sm font-medium transition-all cursor-pointer flex items-center justify-center ${
                          isConnected
                            ? "bg-white text-[#a33b20] border border-gray-300 hover:bg-gray-50"
                            : "bg-[#b89d5f] text-white hover:bg-[#a68c4f]"
                        }`}
                      >
