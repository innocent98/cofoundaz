...
export default function FundingLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: "Pipeline", path: "/funding" },
    { name: "Data room", path: "/funding/data-room" },
    { name: "Cap table", path: "/funding/cap-table" },
    { name: "Grants", path: "/funding/grants" },
    { name: "Term sheets", path: "/funding/term-sheets" },
    { name: "Analytics", path: "/funding/analytics" },
  ];
      ...
      <div className="flex flex-col min-h-full min-w-0 bg-[#f5f7f5] text-[#2c3531] font-body w-full">
        ...
        <div className="sticky top-0 z-40 bg-white border-b border-sage-200/85 shadow-card w-full">
          <header className="flex w-full items-center justify-between gap-2 px-6 py-3">
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              <div className="flex min-w-0 items-center gap-1 text-sm text-sage-500">
                ...
                <span className="truncate font-bold text-[#1E2923] text-base">Funding Hub</span>
              </div>
            </div>
          ...
          <nav className="flex items-center gap-2 overflow-x-auto whitespace-nowrap scrollbar-none bg-neutral-50 px-6 py-2">
            {navItems.map((tab) => {
              const isActive = pathname === tab.path || (tab.path !== "/funding" && pathname?.startsWith(tab.path));
              return (
                <button
                  key={tab.name}
